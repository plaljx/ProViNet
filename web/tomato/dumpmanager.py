...
class ErrorDumpForm(BootstrapForm):
	...
	dump_id = forms.CharField(max_length=255,help_text=_("The description for the errorgroup. This is also its name in the errorgroup list."), widget=forms.HiddenInput())
...
class ErrorGroupForm(BootstrapForm):
	description = forms.CharField(max_length=255,help_text=_("The description for the errorgroup. This is also its name in the errorgroup list."))
	buttons = Buttons.cancel_add
	def __init__(self, api, *args, **kwargs):
...
def group_edit(api, request, group_id):
	if request.method=='POST':
		...
		UserError.check(group_id, UserError.INVALID_DATA, _("Form transmission failed."))
